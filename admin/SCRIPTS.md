...
**Graceful stop.** Stop and Restart first send a BattlEye RCON `#shutdown` and wait up to 30s for the server to exit on its own, so DayZ flushes its final CE save; only if that fails (RCON off, or no clean exit) do they force-kill. Force-killing during the periodic `[IdleMode]` save is the classic cause of "Serious stream damage" in `storage_*/data/*.bin`, so enable RCON on every map you run. **Start** is gated by a pre-flight (`preflight_on_start`, default on) that refuses to launch a map with a missing server config, mission folder, or mod folders — so it won't boot into a crash.

**Storage backups & health.** Restart backs up the map's `storage_*` in the stopped window (consistent, just-saved state) to `local_backups/storage/<map>/` with rotation; the **Back up map storage** action does it on demand. `GET /api/storage/health?map=` and `triage_latest_logs.py` report stream-damage events and failed-item counts so corruption is visible, not silent. The high-risk **Restore map storage** action (`restore_storage.py`, confirm `RESTORE`) overwrites live persistence from a chosen/newest backup after archiving the current state first — stop the map before restoring. The keep-alive **Watchdog** records the likely crash reason (`lastCrashReason`) and takes a forensic storage backup before relaunching a crashed server.
